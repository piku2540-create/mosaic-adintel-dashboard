...
function computeDaysLive(ad: ParsedAd, todayIso = new Date().toISOString().slice(0, 10)): number {
...
export function computeBrandSummaries(ads: ParsedAd[], brandFilter: string[]): BrandSummary[] {
  const byBrand = new Map<string, ParsedAd[]>();
  for (const ad of ads) {
    if (brandFilter.length && !brandFilter.includes(ad.brandName)) continue;
  ...
  for (const [brandName, list] of byBrand) {
    ...
    let totalDays = 0;
    let longest: ParsedAd | null = null;
    let maxDays = 0;
    ...
    for (const ad of list) {
      ...
      if (ad.adType) byAdType[ad.adType] = (byAdType[ad.adType] || 0) + 1;
      if (ad.hookType) byHookType[ad.hookType] = (byHookType[ad.hookType] || 0) + 1;
      totalDays += daysLive;
      if (daysLive > maxDays) {
        maxDays = daysLive;
        longest = ad;
...
export function getLongevityAds(ads: ParsedAd[], topN: number, brandFilter: string[]): LongevityAd[] {
...
export function detectGaps(ads: ParsedAd[]): GapOpportunity[] {
  const gaps: GapOpportunity[] = [];
  if (!ads.length) return gaps;
  ...
  const brandsFor = (key: keyof ParsedAd, value: string) => {
    const lower = value.toLowerCase();
    const set = new Set<string>();
    for (const ad of ads) {
      const raw = (ad[key] as string | undefined)?.trim().toLowerCase();
      if (raw === lower) {
  ...
  for (const emo of canonicalEmotions) {
    ...
    if (share < 0.1) {
      gaps.push({
        ...
        opportunity: `Only ${(share * 100).toFixed(
          1
        )}% of ads use "${emo}" as the primary emotional trigger. Test more creatives that lean into this emotion.`,
        confidence: 'medium',
      });
    }
  }
  ...
  if (scrollMap.size) {
    ...
    if (uniqueScrolls.length === 1) {
      ...
      gaps.push({
        ...
        opportunity:
          'All ads rely on one scroll-stopper type. Introduce variation (e.g. pattern-interrupt visuals, bold text frames, creator-led hooks).',
        confidence: 'medium',
      });
    }
  }
  ...
  if (offerMap.size) {
    ...
    if (share > 0.4) {
      gaps.push({
        ...
        opportunity: `"${topOffer}" appears in ${(share * 100).toFixed(
          1
        )}% of ads. Test non-discount or value-led offers to cut through.`,
        confidence: 'medium',
      });
    }
  }
  ...
  if (toneMap.size) {
    ...
    if (share > 0.5) {
      gaps.push({
        ...
        opportunity: `"${topTone}" tone is used in ${(share * 100).toFixed(
          1
        )}% of ads. Introduce contrasting tones (e.g. authority, humor, or empathy) for variety.`,
        confidence: 'medium',
      });
    }
  }
  ...
  if (painMap.size >= 2) {
    ...
    if (share > 0.6) {
      gaps.push({
        ...
        opportunity: `"${topPain}" appears in ${(share * 100).toFixed(
          1
        )}% of ads. Explore secondary pain points from research (e.g. stress, self-confidence, convenience) to open new angles.`,
        confidence: 'medium',
      });
    }
  }
...
export function computeCreativeStrength(filteredData: ParsedAd[]): CreativeStrengthResult {
  const items: { ad: ParsedAd; creativeStrengthScore: number; creativeStrengthPercentile: number }[] = [];
  ...
  const byFormatSum: Record<string, { total: number; count: number }> = {};
  const byFunnelSum: Record<string, { total: number; count: number }> = {};
  ...
  const avgByFunnel: Record<string, number> = {};
  ...
  for (const ad of filteredData) {
    ...
    const raw =
      daysPercentile * 0.5 + activeScore * 0.2 + formatNorm * 0.15 + funnelNorm * 0.15;
  ...
  const byBrandSum: Record<string, { sum: number; count: number }> = {};
  ...
  const byFormatScore: Record<string, { sum: number; count: number }> = {};
  for (const { ad, creativeStrengthScore } of items) {
  ...
  if (sorted.length >= 1) {
    insights.push(
      `Strongest creative: ${sorted[0].ad.brandName} (score ${sorted[0].creativeStrengthScore}) — ${sorted[0].ad.adCreativeType}, ${sorted[0].ad.messageTheme || 'N/A'}.`
    );
  }
...
export function computeAggressionScore(filteredData: ParsedAd[]): CompetitorAggressionResult {
  const insights: string[] = [];
  if (filteredData.length === 0) {
    return { brands: [], insights: [] };
  }

  const byBrand = new Map<string, { ads: ParsedAd[]; months: Set<string> }>();
  for (const ad of filteredData) {
    const b = ad.brandName || 'Unknown';
    if (!byBrand.has(b)) byBrand.set(b, { ads: [], months: new Set() });
    const rec = byBrand.get(b)!;
    rec.ads.push(ad);
    const dateStr = ad.adFirstSeenDate || ad.startDate || '';
    if (dateStr.length >= 7) rec.months.add(dateStr.slice(0, 7));
  }

  const rows: AggressionBrandRow[] = [];
  ...
  for (const row of rows) {
    ...
    const raw =
      volumeScore * 0.35 + churnScore * 0.25 + velocityScore * 0.25 + videoScore * 0.15;
  ...
  for (let i = 0; i < rows.length; i++) {
    const pct = (i + 1) / n;
    if (pct <= 0.25) rows[i].tier = 'Top 25%';
    else if (pct <= 0.5) rows[i].tier = 'High';
    else if (pct <= 0.75) rows[i].tier = 'Medium';
    else if (pct < 1) rows[i].tier = 'Low';
    else rows[i].tier = 'Bottom 25%';
  }

  if (rows.length > 0) {
    insights.push(
      `Most aggressive brand in this set: ${rows[0].brandName} (score ${rows[0].aggressionScore}) — ${rows[0].totalAds} ads, ${rows[0].avgDaysLive} avg days live.`
    );
...
export function computeEmotionFormatMatrix(filteredData: ParsedAd[]): EmotionFormatMatrixResult {
  const insights: string[] = [];
  const key = (emo: string, fmt: string) => `${emo}|||${fmt}`;
  const cellsMap = new Map<
    string,
    { emotionalTrigger: string; adFormat: string; daysSum: number; count: number; activeCount: number }
  >();
  ...
  for (const ad of filteredData) {
    ...
    c.count += 1;
    if (ad.isCurrentlyActive) c.activeCount += 1;
  }

  const cells: EmotionFormatCell[] = [];
  ...
  if (underused.length > 0) {
    const u = underused[0];
    insights.push(
      `Underused + high longevity: "${u.emotionalTrigger}" × ${u.adFormat} (${u.count} ads, ${u.avgDaysLive} avg days live).`
    );
  }
  if (overused.length > 0) {
    const o = overused[0];
    insights.push(
      `Overused + low longevity: "${o.emotionalTrigger}" × ${o.adFormat} (${o.count} ads, ${o.avgDaysLive} avg days live).`
    );
  }
...
export function computePainOfferAlignment(filteredData: ParsedAd[]): PainOfferAlignmentResult {
  const insights: string[] = [];
  ...
  for (const ad of filteredData) {
    ...
    if (hasPain) {
      const b = ad.brandName || 'Unknown';
      if (!byBrand.has(b)) byBrand.set(b, { aligned: 0, withPain: 0 });
      const rec = byBrand.get(b)!;
      rec.withPain += 1;
      if (aligned) rec.aligned += 1;
    }
    if (hasPain && hasOffer) {
  ...
  const byBrandList: { brandName: string; alignedCount: number; totalWithPainPoint: number; alignmentPct: number }[] = [];
  ...
  if (topBrands.length > 0) {
    insights.push(
      `Best pain–offer alignment: ${topBrands[0].brandName} (${topBrands[0].alignmentPct}% of ads with pain point have an offer).`
    );
  }
  if (topCombos.length > 0) {
    insights.push(
      `Top combination: "${topCombos[0].pain}" + "${topCombos[0].offer}" (${topCombos[0].count} ads).`
    );
  }
...
export async function generateAIInsights(
  ...
  try {
    ...
    const parsed = JSON.parse(content) as { insights: AIInsightItem[]; weeklyBrief: string[] };
    return {
