...
import type { ParsedAd } from '../types/index.js';
...
type RawMetaAdRow = Record<string, unknown>;

function getString(row: RawMetaAdRow, key: string): string {
  const v = row[key];
  if (v === null || v === undefined) return '';
  return String(v).trim();
}

export function parseMetaAdsJson(input: unknown): ParsedAd[] {
