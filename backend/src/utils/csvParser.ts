...
import type { RawAdRow } from '../types/index.js';
import type { ParsedAd, AdCreativeType } from '../types/index.js';
import { normalizeMessageThemeAndFormat } from './categorization.js';
import {
  computeDaysLive,
  normalizeMosaicBrandCategory,
  parseAdFormat,
  parseBool,
  parseDateFlexible,
} from './adNormalize.js';
...
export function parseCSVBuffer(buffer: Buffer): ParsedAd[] {
  const text = buffer.toString('utf-8');
  const rows = parse(text, { columns: true, skip_empty_lines: true, trim: true }) as RawAdRow[];
