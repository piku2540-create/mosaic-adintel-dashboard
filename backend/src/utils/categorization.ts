...
interface Input {
  messageThemeFromCsv: string;
  hookTypeFromCsv: string;
  adTypeFromCsv: string;
  hookText: string;
...
function normalizeTheme(csv: string, hookType: string, hookText: string): string {
  const theme = csv || `${hookType ?? ''} ${hookText ?? ''}`;
  const normalized = theme?.trim().toLowerCase() || '';

  const includesAny = (keywords: string[]) => keywords.some((kw) => normalized.includes(kw));
...
function normalizeFormat(adTypeCsv: string, hookText: string, adCreativeType: AdCreativeType): string {
  if (adTypeCsv?.trim()) {
...
export function normalizeMessageThemeAndFormat(input: Input): { messageTheme: string; creativeFormat: string } {
  return {
    messageTheme: normalizeTheme(
      input.messageThemeFromCsv,
      input.hookTypeFromCsv,
      input.hookText
    ),
    creativeFormat: normalizeFormat(
      input.adTypeFromCsv,
      input.hookText,
      input.adCreativeType
    ),
  };
}
